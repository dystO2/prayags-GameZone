...
document.addEventListener('DOMContentLoaded', () => {
  ...
  squares.forEach((item, i) => {
    item.addEventListener("click", function(){
      ...
      //check if square exactly below is taken
      if(squares[i+7].classList.contains("taken")) {
        item.classList.add("taken");
        if(currentPlayer === 1) {
          //add player 1 coin there
          item.classList.add("player-one");
          item.classList.add("disabled");
          currentPlayer = 2;
        }
        else if(currentPlayer === 2) {
          //add player 2 coin there
          item.classList.add("player-two");
          item.classList.add("disabled");
          currentPlayer = 1;
        }
      }
      ...
      currentPlayerDisplay.innerHTML = "Current Player: Player " + currentPlayer;

      //check if any winning combination satisfied at end of this turn
      winningArrays.forEach((item, i) => {
        //assign 4 squares to each particular combination
        const square1 = squares[item[0]];
        const square2 = squares[item[1]];
        const square3 = squares[item[2]];
        const square4 = squares[item[3]];

        //check if those squares are all taken by player 1
        if(square1.classList.contains('player-one') && square2.classList.contains('player-one') && square3.classList.contains('player-one') && square4.classList.contains('player-one')) {
                //player 1 wins
                currentPlayerDisplay.innerHTML = 'Player 1 wins!';
                grid.classList.add("disabled");
        }

        //check if those squares are all taken by player 2
                ...
                //player 2 wins
                currentPlayerDisplay.innerHTML = 'Player 2 wins!';
                grid.classList.add("disabled");
        }
      });
    });
